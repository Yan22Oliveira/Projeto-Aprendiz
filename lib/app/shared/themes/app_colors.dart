import  'package:flutter/material.dart';

class AppColors{
  static const primary     = Color(0xFF922424);
  static const secundary   = Color(0xFF202020);
  static const orange      = Color(0xFFF7972E);
  static const creme       = Color(0xFFFFFFDB);
  static const verde       = Color(0xFF319124);
  static const delete      = Color(0xFFE83F5B);
  static const stroke      = Color(0xFFE3E3E6);
  static const shape       = Color(0xFFFAFAFC);
  static const background  = Color(0xFFFFFFFF);
  static const black       = Color(0xFF000000);
  static const black87     = Color(0xDD000000);
  static const cinzaClaro  = Color(0xFFEFECEC);
  static const cinzaEscuro = Color(0xFFB7B7B7);
}